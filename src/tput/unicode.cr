class Tput
  ...
  module Unicode
    ...
    def pad(cell : String, width : Int32, align : AlignFlag?) : String
    ...
    def needs_cluster?(base : Char, nxt : Char?) : Bool
      ...
      # `\'e\'` followed by a combining mark (e.g. U+0301, NFD "e\u0301") has
    ...
    def extend_grapheme(content, ci : Int32, base : Char) : Tuple(String, Int32)
      ...
      if regional_indicator?(base)
        if (c = content[ci]?) && regional_indicator?(c)
          g << c
          ci += 1
        end
        return {g.to_s, ci}
